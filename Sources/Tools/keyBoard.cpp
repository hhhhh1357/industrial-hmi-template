...
void KeyBoard::focusChanged(QWidget *, QWidget *nowWidget)
{
    ...
    {
        if (nowWidget->inherits("QLineEdit") && nowWidget->isEnabled() &&(!(((QLineEdit *)nowWidget)->isReadOnly())))
        {
            QLineEdit * nowLineEdit = (QLineEdit *)nowWidget;

            if (currentLineEdit!=nowLineEdit)
            {
               currentLineEdit = nowLineEdit;
               this->isShowed = true;
            }
            else
            {
            ...
            {
                // 靠上居中显示
                movePoint = QPoint(deskWidth/2 - frmWidth/2, QCursor::pos().y()-frmHeight-currentLineEdit->height()-64);
            }
            else
            {
                // 靠下居中显示
                movePoint = QPoint(deskWidth/2 - frmWidth/2, QCursor::pos().y()+currentLineEdit->height()+64);
            }
