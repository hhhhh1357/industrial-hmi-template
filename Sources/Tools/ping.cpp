...
{       
	...
	{   
        ...
        ret = unpackIcmp(m_recvpacket, len, &icmpEchoReply);
//printf("unpackIcmp return=%d\n",ret);
//		if(ret==false) {
//			//retry again
//			recvCount--;
//			continue;
...
//	}
    ...
    ret = recvPacket(pingResult);  /*接收所有ICMP报文*/
...
bool Ping::CheckNetWork(std::string host)
{
    bool ret = true;
    PingResult pingResult;

    for (int count = 1; count <= 2; count++) {
        ret = ret && this->ping(host, 1, pingResult);
        //printf("CheckNetWork...,ret=%d\n", ret);
    }
    return ret;
}

bool Ping::CheckNetWork(std::string hosts[], int n)
{
    bool ret = true;
    PingResult pingResult;
    ...
    int m = qrand() % (n);
    ret = ret && this->ping(hosts[m], 1, pingResult);
    if (!ret)
    {
        m = (m+2)%n;
        ret = ret && this->ping(hosts[m], 1, pingResult);
...
bool Ping::CheckNetWork(QList<QString> hosts)
{
    int n = hosts.length();
    bool ret = true;
    PingResult pingResult;
    ...
    qsrand(time.msec()+time.second()*1000);
    int m = qrand() % (n);
    ret = ret && this->ping(hosts.at(m).toStdString(), 1, pingResult);
    if (!ret)
    {
        m = (m+1)%n;
        ret = ret || this->ping(hosts.at(m).toStdString(), 1, pingResult);
...
bool Ping::CheckNetWork( )
{

//return true;  // 测试用。绕开PC
    ...
    for (int count = 1; count <= 2; count++) {
        ret = ret && this->ping(hosts[n], 1, pingResult);
        printf("ping::CheckNetwork..., ping %s\n", hosts[n]);
        //printf("CheckNetWork...,ret=%d\n", ret);
    }
    return ret;
}
