...
SubWorker3080::SubWorker3080(QString ip, QString pw, QString mn, SqliteTools *sqlTools, QObject *parent):SubWorkerBase(ip, pw, mn,sqlTools, parent)
{

}
SubWorker3080::~SubWorker3080()
{
    //qDebug()<<__FILE__ << ", " << __func__<<" was called !"<<endl;
}


void SubWorker3080::doWork(Task task)
{
    ...
    if(isValidCmd){
        ...
        if(isNewDialog && taskSource == TaskDirection::CLOUD_REQUEST){ //来自云端的新的命令，需要一个回复
            ...
            if(r){
                task.req.action = PLCAction::CMD_3080;
...
QString SubWorker3080::buildCloudCmd(Task &task,bool &shouldDone)
{
    return "";
