...
#include "SubWorker1072.h"

#include <Headers/BLL/BLocalService.h>
extern BLocalService * localService;
SubWorker1072::SubWorker1072(QString ip, QString pw, QString mn, SqliteTools *sqlTools, QObject *parent):SubWorkerBase(ip, pw,  mn,sqlTools, parent)
{

}

SubWorker1072::~SubWorker1072()
{
    //qDebug()<<__FILE__ << ", " << __func__<<" was called !"<<endl;
}

void SubWorker1072::doWork(Task task)
{
    ...
    if(isValidCmd){
        ...
        if(isNewDialog && taskSource==TaskDirection::CLOUD_REQUEST){
            ...
            localService->setLocalPassword(PWD);
...
QString SubWorker1072::buildCloudCmd(Task &task, bool &shouldDone)
{
    return "";
}
