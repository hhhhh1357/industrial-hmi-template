...
SubWorker3086::SubWorker3086(QString ip, QString pw, QString mn, SqliteTools *sqlTools, QObject *parent):SubWorkerBase(ip, pw, mn,sqlTools, parent)
//SubWorker3086::SubWorker3086(QString ip, QObject *parent):SubWorkerBase(ip,  parent)
{

}
SubWorker3086::~SubWorker3086()
{
    //qDebug()<<__FILE__ << ", " << __func__<<" was called !"<<endl;
}


void SubWorker3086::doWork(Task task)
{
    ...
    if(isValidCmd){
        ...
        if(isNewDialog && taskSource == TaskDirection::CLOUD_REQUEST){ //来自云端的新的命令，需要一个回复
            ...
            if(r){
                task.req.action = PLCAction::CMD_3086;
...
QString SubWorker3086::buildCloudCmd(Task &task,bool &shouldDone)
{    
    return "";
