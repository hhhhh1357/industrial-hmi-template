//2011 取污染物实时数据（上位机发起）&& 上传污染物实时数据（下位机发起）

#include "SubWorker2011.h"
...
SubWorker2011::SubWorker2011(QString ip, QString pw, QString mn, SqliteTools *sqlTools, QObject *parent):SubWorkerBase(ip, pw, mn,sqlTools, parent)
//SubWorker2011::SubWorker2011(QString ip, QObject *parent):SubWorkerBase(ip,  parent)
{

}
SubWorker2011::~SubWorker2011()
{
    //qDebug()<<__FILE__ << ", " << __func__<<" was called !"<<endl;
}


void SubWorker2011::doWork(Task task)
{
    PackingTools pt;
    //================================================================================================================================================
    //先要验证上位机请求命令是否有效
    ...
    if(isValidCmd){
        ...
        else if(isNewDialog && taskSource == TaskDirection::SELF_REPORT){//主动上传
            this->QN1 = QN;
            sqlTools->newDialogRecord(this->QN1, (qlonglong)this, taskSource, this->IP);
            //2. 上传的实时数据共三种，分别是：
            //a). 上传噪声声级实时数据 标准格式：QN=20160801085857223;ST=23;CN=2011;PW=123456;MN=010000A8900016F000169DC0;Flag=5;CP=&&DataTime=20160801085857; LA-Rtd=50.1&&
            //b). 上传污染物实时数据 标准格式：QN=20160801085857223;ST=32;CN=2011;PW=123456;MN=010000A8900016F000169DC0;Flag=5;CP=&&DataTime=20160801085857;w01001-Rtd=7.1,w01001-Flag=N;w01018-SampleTime=20160801070000,01018-Rtd=2.2,w01018-Flag=N,w01018-EFlag=A01;…&&
            //c). 上传工况实时数据 标准格式：QN=20160801085857223;ST=52;CN=2011;PW=123456;MN=010000A8900016F000169DC0;Flag=5;CP=&&DataTime=20160801085857;e01001-Rtd=1.1;e01001-Rtd=2.2…&&
            QString msg = pt.CloudCmdBuilder(dpd, &isOK);
            this->LatestSendCmd = task.dataPacket.data.SelfReportCmd;
            this->startTimer();
            emit toCloud(msg);
        }
        else   //主动上传，接收到上位机的应答后，结束SubWorker任务对象
        {
            this->stopTimer();
            emit isDone(this->QN1);
        }
...
QString SubWorker2011::buildCloudCmd(Task &task,bool &shouldDone)
{
/*
    PackingTools pt;
